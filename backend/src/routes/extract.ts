...
const EXTRACTION_SCHEMA = {
  ...
  required: ['events', 'eliminated', 'summary', 'warnings'],
  properties: {
    events: {
      type: 'array',
      items: {
...
- Only use rules that exist in the list. Never invent players.`;
    ...
    content.push({ type: 'text', text: `Extract every scoring event for Episode ${episode} from the sources above. Cover challenges (tribe and individual), tribal council votes (who received how many votes, who voted with the majority), idols and advantages found or played, journeys, merge/jury milestones if this is that episode, and who was eliminated. Return the structured result.` });
